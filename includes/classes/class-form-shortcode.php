...
class Form_Shortcode {
	...
	/**
	 * A plan object as per the Flutterwave API Fetch request
	 * @link https://flutterwave.com/docs/api/plan
	 * @var object
	 */
	public $plan = false;
	...
	public function __construct() {
		...
		add_shortcode( 'flutterwave_form', [ $this, 'form_shortcode' ] );
		add_shortcode( 'pff-flutterwave', [ $this, 'form_shortcode' ] );
	}
	...
	public function form_shortcode( $atts ) {
		...
		if ( ! $public_key ) {
			$settings_link = esc_url( get_admin_url( null, 'edit.php?post_type=flutterwave_form&page=settings' ) );
			return sprintf(
				'<h5>%s <a href="%s">%s</a></h5>',
				esc_html__( 'You must set your Flutterwave API keys first', 'pff-flutterwave' ),
				esc_url( $settings_link ),
				esc_html__( 'settings', 'pff-flutterwave' )
		...
		if ( $id > 0 ) {
			$obj = get_post( $id );
			if ( null !== $obj && 'flutterwave_form' === get_post_type( $obj ) ) {
				...
				if ( $show_form ) {
					...
					if ( 'yes' === $this->meta['useinventory'] && 0 >= $this->stock ) {
						$html[] = '<h1>' . esc_html__( 'Out of Stock', 'pff-flutterwave' ) . '</h1>';
					} else {
						// Form title
						...
						// Start form output
						$html[] = '<form version="' . esc_attr( PFF_FLUTTERWAVE_VERSION ) . '" enctype="multipart/form-data" action="' . esc_url( admin_url( 'admin-ajax.php' ) ) . '" method="post" class="flutterwave-form j-forms" novalidate>
							<div class="j-row">';
					...
					$html[] = '<h5>' . esc_html__( 'You must be logged in to make a payment.', 'pff-flutterwave' ) . '</h5>';
				}
			} else {
				$html[] = '<h5>' . esc_html__( 'Invalid Flutterwave form ID or the form does not exist.', 'pff-flutterwave' ) . '</h5>';
			}
		} else {
			$html[] = '<h5>' . esc_html__( 'No Flutterwave form ID provided.', 'pff-flutterwave' ) . '</h5>';
		}
	...
	public function set_meta_data( $obj ) {
		...
		$this->meta['planerrorcode'] = esc_html__( 'Input Correct Recurring Plan Code', 'pff-flutterwave' );

		if ( 'plan' === $this->meta['recur'] ) {
			if ( '' === $this->meta['recurplan'] ) {
				$this->show_btn = false;
			} else {
				$this->plan = pff_flutterwave()->classes['request-plan']->fetch_plan( $this->meta['recurplan'] );
				if ( false !== $this->plan && isset( $this->plan->data->amount ) ) {
					$this->has_plan = true;
					// Flutterwave plan amounts are in major units already — no /100 conversion.
					$this->meta['planamount'] = $this->plan->data->amount;
				} else {
					$this->show_btn = false;
				}
			}
	...
	public function get_hidden_fields() {
		// Hidden inputs
		$html = '<input type="hidden" name="action" value="pff_flutterwave_submit_action">
				...
				wp_nonce_field( 'pff-flutterwave-invoice', 'pf-nonce', true, false );
				;
		return $html;
	}
	...
	public function get_fullname_field() {
		$html = '<div class="span12 unit">
			<label class="label">' . esc_html__( 'Full Name', 'pff-flutterwave' ) . ' <span>*</span></label>
			<div class="input">
				<input type="text" name="pf-fname" placeholder="' . esc_html__( 'First & Last Name', 'pff-flutterwave' ) . '" value="' . esc_attr( $this->user['fullname'] ) . '" required>
			</div>
		</div>';
		return $html;
	}
	...
	public function get_email_field() {
		$html = '<div class="span12 unit">
			<label class="label">' . esc_html__( 'Email', 'pff-flutterwave' ) . ' <span>*</span></label>
			<div class="input">
				<input type="email" name="pf-pemail" placeholder="' . esc_html__( 'Enter Email Address', 'pff-flutterwave' ) . '" id="pf-email" value="' . esc_attr( $this->user['email'] ) . '" ' . ( $this->meta['loggedin'] == 'yes' ? 'readonly' : '' ) . ' required>
			</div>
		</div>';
		return $html;
	}
		...
		$html[] = '<div class="span12 unit">
			...
			if ( 0 === $this->meta['usevariableamount'] ) {
				...
				if ( '' === $this->meta['variableamount'] || 0 === $this->meta['variableamount'] || ! is_array( $this->meta['paymentoptions'] ) ) {
					$html[] = esc_html__( 'Form Error, set variable amount string', 'pff-flutterwave' );
	...
	public function get_form_footer() {
		...
		$html[] = '<div class="span12 unit">
			...
			<br />
			<img src="' . esc_url( PFF_FLUTTERWAVE_PLUGIN_URL . '/assets/images/logos@2x.png' ) . '" alt="cardlogos" class="flutterwave-cardlogos size-full wp-image-1096" />
	...
	public function get_retry_form( $code = '' ) {
		...
		if ( false !== $record ) {
			...
			$html[] = '<div class="divider-text gap-top-20 gap-bottom-45">
							<span>' . esc_html__( 'Payment Invoice', 'pff-flutterwave' ) . '</span>
						</div>';
			
			$html[] = '<div class="j-row">';

			$html[] = '<div class="span12 unit">
							<label class="label inline">' . esc_html__( 'Email:', 'pff-flutterwave' ) . '</label>
							<strong><a href="mailto:' . esc_attr( $record->email ) . '">' . esc_html( $record->email ) . '</a></strong>
						</div>';

			$html[] = '<div class="span12 unit">
							<label class="label inline">' . esc_html__( 'Amount:', 'pff-flutterwave' ) . '</label>
			...
			$html[] = '<div class="span12 unit">
							<label class="label inline">' . esc_html__( 'Date:', 'pff-flutterwave' ) . '</label>
							<strong>' . esc_html( $record->created_at ) . '</strong>
						</div>';

			if ( 1 === intval( $record->paid ) ) {
				$html[] = '<div class="span12 unit">
								<label class="label inline">' . esc_html__( 'Payment Status:', 'pff-flutterwave' ) . '</label>
								<strong>' . esc_html__( 'Successful', 'pff-flutterwave' ) . '</strong>
							</div>';
			}
			...
			$html[] = '<small><span style="color: red;">*</span> ' . esc_html__( 'are compulsory', 'pff-flutterwave' ) . '</small><br>';
			$html[] = '<img class="flutterwave-cardlogos size-full wp-image-1096" alt="cardlogos" src="' . esc_url( PFF_FLUTTERWAVE_PLUGIN_URL . '/assets/images/logos@2x.png' ) . '">';
			if ( 0 === intval( $record->paid ) ) {
				$html[] = '<button type="submit" class="primary-btn" id="submitbtn">' . esc_html__( 'Retry Payment', 'pff-flutterwave' ) . '</button>';
			}
			...
			$html[] = esc_html__( 'Invoice code invalid', 'pff-flutterwave' );
		}
