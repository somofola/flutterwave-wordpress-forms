...
class Field_Shortcodes {
	...
	public function text_field( $atts ) {
		$atts = shortcode_atts(
			array(
				'name'     => esc_html__( 'Title', 'pff-flutterwave' ),
				'required' => '0',
			),
			$atts,
			'text'
		...
		$name     = sanitize_text_field( sprintf( esc_attr__( 'Enter %s', 'pff-flutterwave' ), $atts['name'] ) );
	...
	public function input_field( $atts ) {
		...
		$code .= esc_html__( 'Browse', 'pff-flutterwave' );
		...
		$code .= '</div>';
		$code .= '<input type="text" id="' . esc_attr( $textInputId ) . '" readonly="" placeholder="' . esc_attr__( 'No file selected', 'pff-flutterwave' ) . '">';
		$code .= '</div></div>';
