...
class Retry_Submit {
	...
	public function __construct() {
		add_action( 'wp_ajax_pff_flutterwave_retry_action', [ $this, 'retry_action' ] );
		add_action( 'wp_ajax_nopriv_pff_flutterwave_retry_action', [ $this, 'retry_action' ] );
	}
	...
	public function retry_action() {
		if ( ! isset( $_POST['pf-nonce'] ) || false === wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['pf-nonce'] ) ), 'pff-flutterwave-retry' ) ) {
			$response = array(
				'result'  => 'failed',
				'message' => esc_html__( 'Nonce verification is required.', 'pff-flutterwave' ),
			);
		...
		if ( isset( $_POST['code'] ) && '' !== trim( wp_unslash( $_POST['code'] ) ) ) {
			...
			$response = array(
				'result'  => 'failed',
				'message' => esc_html__( 'Code is required', 'pff-flutterwave' ),
			);
			// Exit here, for not processing further because of the error.
			exit( wp_json_encode( $response ) );
		}
		...
		$response = array(
			...
			'name'          => $fullname,
			// Flutterwave expects amount in major units — no kobo multiplication.
			...
			'title'         => get_the_title( $this->retry_meta->post_id ),
			'description'   => get_the_title( $this->retry_meta->post_id ),
		);
		...
		$response['retryNonce'] = wp_create_nonce( 'pff-flutterwave-retry' );
	...
	/**
	 * Generate a unique Flutterwave code that does not yet exist in the database.
	 *
