...
class Email_Receipt_Owner extends Email {
	...
	public function __construct() {
		add_action( 'pff_flutterwave_send_receipt_owner', [ $this, 'send_receipt_owner' ], 11, 7 );
	}
	...
	function send_receipt_owner( $form_id, $currency, $amount, $name, $email, $code, $metadata ) {
		...
		// Custom Values
		$this->subject     = esc_html__( 'You just received a payment' , 'pff-flutterwave' );
													...
													<tr>
														<td class="column_cell font_default" align="center" valign="top" style="padding:16px;font-family:Helvetica,Arial,sans-serif;font-size:15px;text-align:left;vertical-align:top;color:#888;padding-top:0;padding-bottom:0">
															...
															<p style="font-family:Helvetica,Arial,sans-serif;font-size:15px;line-height:23px;margin-top:8px;margin-bottom:16px">
																<?php esc_html_e( 'Amount', 'pff-flutterwave' ); ?> <strong>: <?php echo esc_html( $this->currency ) . ' ' . number_format( $this->amount ); ?></strong><br>
																<?php esc_html_e( 'Email', 'pff-flutterwave' ); ?> <strong>: <?php echo esc_html( $this->html_email ); ?></strong><br>
																<?php
																	...
																	if ( array_key_exists( "0", $new ) ) {
																		foreach ( $new as $key => $item ) {
																			if ( 'text' === $item->type ) {
																				...
																				echo esc_html( $item->display_name ) . '<strong> : <a target="_blank" href="' . esc_url( $item->value ) . '">' . esc_html__( 'link', 'pff-flutterwave' ) . '</a></strong><br>';
																			}
																		}
																	} else {
																		if ( count( $new ) > 0 ) {
																...
																?>
																<?php esc_html_e( 'Transaction code', 'pff-flutterwave' ); ?>: <strong><?php echo esc_html( $this->code ); ?></strong><br>
															</p>
														</td>
													</tr>
												</tbody>
